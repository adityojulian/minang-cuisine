...
const create = asyncHandler(async (req, res) => {
	try {
		...
		const { first_name, last_name, unique_id, password, contact } = req.body;
		// REQ CREATE WALLET
		const wallet_body = {
...
const getItems = asyncHandler(async (req, res) => {
	try {
		...
		resolvepromises(querySnapshot.docs).then((results) => res.json(results));
	} catch (err) {
		res.json(err);
	}
});
